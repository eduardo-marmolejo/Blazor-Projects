    ...
    public class UsuarioServicio: IUsuarioServicio
    {
        ...
        public async Task<SesionDTO> Autorizacion(LoginDTO modelo)
        {
            try
            {
                ...
                {
                    throw new TaskCanceledException("No se encontraron coinciddencias");
                }
            }
            catch(Exception ex)
            {
        ...
        public async Task<List<UsuarioDTO>> Lista(string rol, string buscar)
        {
            try
            {
                var consulta = _modeloRepositorio.Consultar(p => p.Rol == rol && string.Concat(p.NombreCompleto.ToLower(),
